...
class Dataset(Dataset):
  ...
  def load_files (self, directory, extension = ".jpg"):
    lines = list()
  ...
  def load_data (self):
    self.prepare_mapping_data_coco2014 ()

    ###
    for directory in self.data_opt['images_dir']:
      print (directory)
      if directory.find('train2014') >= 0:
        lines = self.load_files (directory)
        self.save_lines ('files/train2014.txt', lines)
      else:
        lines  = self.load_files (directory)
        slices = int(len(lines) * 0.8)
        valid, test = lines[:slices], lines[slices:]
        self.save_lines ('files/valid2014.txt', valid)
        self.save_lines ('files/test2014.txt', test)
    ...
    self.train = self.load_set('files/train2014.txt', self.data_opt['limits_train'])
    self.valid = self.load_set('files/valid2014.txt', self.data_opt['limits_valid'])
    self.test  = self.load_set('files/test2014.txt',  self.data_opt['limits_test'])
