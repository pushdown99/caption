...
class v1 (tf.keras.Model):
  def __init__(self, config, data_opt, model_opt, verbose = True):
    ...
    #IMAGE_SIZE = (299, 299)
    #model = EfficientNetB0(input_shape=(*IMAGE_SIZE, 3), include_top=False, weights="imagenet")
    #model.trainable = False
    #out   = model.output
    #out   = layers.Reshape((-1, 1280))(out)
    #model = Model(model.input, out)
    model = EfficientNetB0(weights='imagenet')
    #model = layers.GlobalAveragePooling2D()(model.output)
